...
//!Create structure for Train with id and t as its elements
typedef struct Train{
	int id;
	time_t t; 
}Train;

//!Create a nested self referential structure for nodes with Train structure as its element
typedef struct node{
    Train Train;
    struct node *next;
}Node;
...
void enQdeQ(NodePtr topPtr, Train *array, int, int);
void pushPop(NodePtr topPtr, Train *array, int, int);
...
void pushPop(NodePtr topPtr, Train *array, int size, int count)
{
    ...
    int i = count;
    int length = size;
    //! Insert values inside nodes to build stack
    while(length > 0)
    {
        newPtr = malloc(sizeof(Node));
        newPtr->Train.id =  (array+i)->id;
        newPtr->Train.t = (array+i)->t;
        newPtr->next = topPtr;
        topPtr = newPtr;
        i++;
        length--;
    ...
    tempPtr = topPtr;
    tempId = topPtr->Train.id;
    tempT = ctime(&topPtr->Train.t);
    topPtr = topPtr->next;
    free(tempPtr);
...
void enQdeQ(NodePtr headPtr, Train *array, int size, int count)
{
    NodePtr newPtr, tempPtr, tailPtr = NULL;
    int tempId = 0;
    char * tempT;
    int i = count;
    int length = size;
    ...
    tempId = headPtr->Train.id;
    tempT = ctime(&headPtr->Train.t);
    tempPtr = headPtr;
